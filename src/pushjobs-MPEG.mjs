...
async function pushJobs() {
    ...
    ss.spreadsheets.values.get(
        ...
        (err, res) => {
            ...
            processQueue(queueRecords, listOfSpecies, 'records', 'Museu-Goeldi/PA');
            // processQueue(queueInformation, listOfSpecies, 'information', 'Museu-Goeldi/PA');
            ...
            processConservationActionsQueue(queueConservationActions, listOfSpecies, 'Museu-Goeldi/PA');
            processThreatsQueue(queueThreats, listOfSpecies);
            processSpeciesProfileQueue(queueSpeciesProfile, listOfSpecies, 'Museu-Goeldi/PA');
        }
    );
}
...
function processDistributionQueue(queue, listOfSpecies) {
    queue.getJobs().then(jobs => {
        ...
        speciesToAdd.forEach(species => {
            sendPostRequestWithSource('distribution', species, 'Museu-Goeldi/PA');
        });
    });
}
...
function processConservationActionsQueue(queue, listOfSpecies, source) {
    queue.getJobs().then(jobs => {
        const jobNames = jobs.map(job => job.data.species);
        const speciesToAdd = listOfSpecies.filter(species => {
            const pathRecords = `G:/Outros computadores/Meu computador/CNCFlora_data/records/${species}.json`;
        ...
        speciesToAdd.forEach(species => {
            sendPostRequestWithSource('conservationActions', species, source);
        });
    });
}
    ...
    queue.getJobs().then(jobs => {
        ...
        speciesToAdd.forEach(species => {
            ...
            if (isReady && !jobNames.includes(species)) {
                sendPostRequestWithSource('speciesProfile', species, source);
            }
        });
    });
}
